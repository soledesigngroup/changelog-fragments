 ...
 *
 *   1. A `---` header rule before the first day block. `tier1Bounds()` treats the
 *      first `---` in the file as the top of the Tier-1 zone — without one at the
 *      top, the first separator the fold itself writes would become the boundary
 *      and every later entry would be inserted BELOW the newest day.
 ...
 * duplicate dates. Those headings don't match the tooling's `^## YYYY-MM-DD$`
 * regex, so they're simply invisible to it; they age out naturally as condensing
 * rewrites them. Normalizing them would mean merging same-date sections, i.e.
 * editing history, which this deliberately does not do.
 *
 * Every migration is verified lossless: each non-blank line of the original must
 * appear in the output, in the same order. Anything else throws.
 */
...
export function migrateChangelog(text, { projectName = "this project" } = {}) {
  ...
  if (pre.length === 0) {
    ...
    pre.unshift("# Changelog", "")
    added.push("title")
  }
  if (!HR.test(pre[pre.length - 1] ?? "")) {
    pre.push("", "---")
...
function assertLossless(before, after, added) {
  ...
  // Structural lines are only exempt when this run actually added them; a `---`
  // that existed before must still be matched positionally.
  const kept = []
