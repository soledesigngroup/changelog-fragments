...
1. **Write one fragment per session** at `docs/changelog.d/<YYYY-MM-DD>-<slug>-<token>.md`
   ...
   the filename. Inside: `### Category` sections (**Added / Changed / Fixed / Removed / Security**)
   with `- **bold subject** — …` bullets. Link hrefs are written relative to `docs/` (`../src/…`),
   i.e. as they'll read once folded. Run `/changelog-update` to do this properly.
...
3. **Folding is a separate, serialized step** run when the tree is quiet: `{{FOLD_CMD}}`
   ...
   summary tiers into `docs/changelog-archive.md`.
